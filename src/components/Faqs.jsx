...
export function Faqs() {
  return (
    <section
      ...
      className="border-t border-violet-700 bg-black py-20 sm:py-32"
    >
      <Container>
            ...
            <li key={columnIndex}>
              <ul role="list" className="space-y-10">
                {column.map((faq, faqIndex) => (
                  <li key={faqIndex}>
                    <h3 className="text-lg font-semibold leading-6 text-violet-700">
                      {faq.question}
                    </h3>
                    <p className="mt-4 text-sm text-violet-500">{faq.answer}</p>
                  </li>
                ))}
              </ul>
            </li>
